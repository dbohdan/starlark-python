...
def _bind(name: str, value: Any, impl: Callable) -> BuiltinFunction:
    ...
    # `functools.partial` (not a lambda) so the receiver is bound *and*
    # `inspect.signature(impl)` still reports the method's real remaining
    # parameters. The evaluator relies on that to classify an argument-binding
    # TypeError without inspecting Python stack frames.
    return BuiltinFunction(
        name=name,
        impl=functools.partial(impl, value),
        self_repr=type_name,
    )
